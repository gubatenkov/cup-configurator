...
export default function GeometryList() {
  ...
  const [geometryItems, setGeometryItems] = useState<
    { handleAddItem: () => void; icon: LucideIcon }[] | []
  >([])
  ...
  return isMounted ? (
    geometryItems.map(({ handleAddItem, icon: Icon }, index) => (
      <Card
    ...
    <Loader2Icon
      className="absolute left-[calc(50%-1rem)] top-[calc(50%-1rem)] animate-spin"
      size={64}
    />
  )
}
